...
export default function EyeHealthPage() {
  ...
  const [imageType, setImageType] = useState<'left' | 'right' | 'both'>('both');
  ...
  const handleAnalyze = async () => {
    ...
    setAnalyzing(true);
    try {
      const response = await fetch('/api/eye-health', {
        method: 'POST',
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold mb-2">眼部健康检测</h1>
          <p className="text-muted-foreground">通过眼部特征分析健康状况</p>
        </div>

        {!result ? (
          <Card>
            <CardHeader>
              <CardTitle>上传眼部照片</CardTitle>
              <CardDescription>请选择左眼、右眼或双眼进行检测</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium mb-2">选择眼部</label>
                <div className="flex gap-4">
                  ...
                  <Button
                    variant={imageType === 'both' ? 'default' : 'outline'}
                    onClick={() => setImageType('both')}
                  >
                    双眼
                  </Button>
                </div>
